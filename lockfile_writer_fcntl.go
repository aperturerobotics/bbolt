...
func (lf *LockFile) AcquireWriterLock() error {
	...
	for {
		err := syscall.FcntlFlock(uintptr(lf.fd.Fd()), syscall.F_SETLKW, &lock)
		if err == nil {
			return nil
		}
...
func (lf *LockFile) TryAcquireWriterLock() (bool, error) {
	...
	for {
		err := syscall.FcntlFlock(uintptr(lf.fd.Fd()), syscall.F_SETLK, &lock)
		if err == nil {
			return true, nil
		}
		if err == syscall.EINTR {
...
func (lf *LockFile) AcquireCoordinationLock() error {
	...
	for {
		err := syscall.FcntlFlock(uintptr(fd.Fd()), syscall.F_SETLKW, &lock)
		if err == nil {
			return nil
		}
		if err != syscall.EINTR {
...
func (lf *LockFile) TryAcquireCoordinationLock() (bool, error) {
	...
	for {
		err := syscall.FcntlFlock(uintptr(fd.Fd()), syscall.F_SETLK, &lock)
		if err == nil {
			return true, nil
		}
		if err == syscall.EINTR {
